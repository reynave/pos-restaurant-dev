...
@Component({
  ...
  styleUrl: './special-hours.component.css'
})
export class SpecialHoursComponent implements OnInit {
  ...
  httpGet() {
    this.loading = true;
    const url = environment.api + "specialHour/";
  ...
  onDelete() {
    if (confirm("Delete this checklist?")) { 
      this.loading = true;
      const url = environment.api + "specialHour/delete";
      const body = this.items;
      this.http.post<any>(url, body, {
        headers: this.configService.headers(),
      }).subscribe(
  ...
  onSubmit() {
    this.loading = true;
    const url = environment.api + "specialHour/create";
    const body = {
      model: this.model,
    };
    this.http.post<any>(url, body, {
