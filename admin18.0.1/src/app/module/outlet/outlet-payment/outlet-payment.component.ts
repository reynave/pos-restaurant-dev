...
@Component({
  ...
  styleUrl: './outlet-payment.component.css'
})
export class OutletPaymentComponent implements OnInit {
  ...
  httpGet() {
    this.loading = true;
    const url = environment.api + "outlet/payment/";
    this.http.get<any>(url, {
      headers: this.configService.headers(),
    }).subscribe(
      data => {
  ...
  onUpdate() {
    this.loading = true;
    const url = environment.api + "outlet/payment/update";
  ...
  onDelete() {
    if (confirm("Delete this checklist?")) { 
      this.loading = true;
      const url = environment.api + "outlet/payment/delete";
      const body = this.items;
      this.http.post<any>(url, body, {
        headers: this.configService.headers(),
      }).subscribe(
  ...
  onSubmit() {
    this.loading = true;
    const url = environment.api + "outlet/payment/create";
    const body = {
      model: this.model,
    };
    this.http.post<any>(url, body, {
