...
@Component({
  selector: 'app-outlet-cash-type',
  standalone: true,
  imports: [HttpClientModule, CommonModule, FormsModule, NgbDropdownModule, NgbDatepickerModule],
  templateUrl: './outlet-cash-type.component.html',
  styleUrl: './outlet-cash-type.component.css'
})
export class OutletCashTypeComponent implements OnInit {
  ...
  httpGet() {
    this.loading = true;
    const url = environment.api + "outlet/cashType/";
    this.http.get<any>(url, {
      headers: this.configService.headers(),
    }).subscribe(
      data => {
  ...
  onUpdate() {
    this.loading = true;
    const url = environment.api + "outlet/cashType/update";
  ...
  onDelete() {
    if (confirm("Delete this checklist?")) { 
      this.loading = true;
      const url = environment.api + "outlet/cashType/delete";
      const body = this.items;
      this.http.post<any>(url, body, {
        headers: this.configService.headers(),
      }).subscribe(
  ...
  onSubmit() {
    this.loading = true;
    const url = environment.api + "outlet/cashType/create";
    const body = {
      model: this.model,
    };
    this.http.post<any>(url, body, {
