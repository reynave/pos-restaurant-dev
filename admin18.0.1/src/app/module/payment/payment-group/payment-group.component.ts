...
@Component({
  ...
  styleUrl: './payment-group.component.css'
})
export class PaymentGroupComponent implements OnInit {
  ...
  onUpdate() {
    this.loading = true;
    const url = environment.api + "payment/paymentGroup/update";
  ...
  onDelete() {
    if (confirm("Delete this checklist?")) { 
      this.loading = true;
      const url = environment.api + "payment/paymentGroup/delete";
      const body = this.items;
      this.http.post<any>(url, body, {
        headers: this.configService.headers(),
      }).subscribe(
  ...
  onSubmit() {
    this.loading = true;
    const url = environment.api + "payment/paymentGroup/create";
    const body = {
      model: this.model,
    };
    this.http.post<any>(url, body, {
