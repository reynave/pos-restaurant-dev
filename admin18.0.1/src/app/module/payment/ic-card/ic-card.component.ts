...
@Component({
  ...
  styleUrl: './ic-card.component.css'
})
export class IcCardComponent implements OnInit {
  ...
  httpGet() {
    this.loading = true;
    const url = environment.api + "payment/icCard/";
  ...
  onUpdate() {
    this.loading = true;
    const url = environment.api + "payment/icCard/update";
  ...
  onDelete() {
    if (confirm("Delete this checklist?")) { 
      this.loading = true;
      const url = environment.api + "payment/icCard/delete";
      const body = this.items;
      this.http.post<any>(url, body, {
        headers: this.configService.headers(),
      }).subscribe(
  ...
  onSubmit() {
    this.loading = true;
    const url = environment.api + "payment/icCard/create";
    const body = {
      model: this.model,
    };
    this.http.post<any>(url, body, {
