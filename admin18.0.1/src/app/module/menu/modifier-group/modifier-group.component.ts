...
imports: [HttpClientModule, CommonModule, FormsModule, NgbDropdownModule, NgbDatepickerModule],
  ...
  styleUrl: './modifier-group.component.css'
})
export class ModifierGroupComponent implements OnInit {
  ...
  httpGet() {
    this.loading = true;
    const url = environment.api + "menu/modifierGroup/";
  ...
  onUpdate() {
    this.loading = true;
    const url = environment.api + "menu/modifierGroup/update";
  ...
  onDelete() {
    if (confirm("Delete this checklist?")) { 
      this.loading = true;
      const url = environment.api + "menu/modifierGroup/delete";
      const body = this.items;
      this.http.post<any>(url, body, {
        headers: this.configService.headers(),
      }).subscribe(
  ...
  onSubmit() {
    this.loading = true;
    const url = environment.api + "menu/modifierGroup/create";
    const body = {
      model: this.model,
    };
    this.http.post<any>(url, body, {
  ...
  open(content: any) {
   ...
   onDuplicate() {
    ...
    this.loading = true;
    const url = environment.api + 'menu/modifierGroup/duplicate';
    const body = this.items;
