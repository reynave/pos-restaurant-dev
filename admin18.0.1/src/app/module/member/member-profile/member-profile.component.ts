...
export class MemberProfileComponent  implements OnInit {
  ...
  httpGet() {
    this.loading = true;
    const url = environment.api + "member/profile/";
  ...
  onUpdate() {
    this.loading = true;
    const url = environment.api + "member/profile/update";
  ...
  onDelete() {
    if (confirm("Delete this checklist?")) { 
      this.loading = true;
      const url = environment.api + "member/profile/delete";
      const body = this.items;
      this.http.post<any>(url, body, {
        headers: this.configService.headers(),
      }).subscribe(
  ...
  onSubmit() {
    this.loading = true;
    const url = environment.api + "member/profile/create";
    const body = {
      model: this.model,
    };
    this.http.post<any>(url, body, {
