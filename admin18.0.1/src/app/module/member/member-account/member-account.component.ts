...
export class MemberAccountComponent  implements OnInit {
  ...
  httpGet() {
    this.loading = true;
    const url = environment.api + "member/account/";
  ...
  onUpdate() {
    this.loading = true;
    const url = environment.api + "member/account/update";
  ...
  onDelete() {
    if (confirm("Delete this checklist?")) { 
      this.loading = true;
      const url = environment.api + "member/account/delete";
      const body = this.items;
      this.http.post<any>(url, body, {
        headers: this.configService.headers(),
      }).subscribe(
  ...
  onSubmit() {
    this.loading = true;
    const url = environment.api + "member/account/create";
    const body = {
      model: this.model,
    };
    this.http.post<any>(url, body, {
