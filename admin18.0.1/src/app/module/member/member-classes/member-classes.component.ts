...
@Component({
  ...
  styleUrl: './member-classes.component.css'
})
export class MemberClassesComponent implements OnInit {
  ...
  httpGet() {
    this.loading = true;
    const url = environment.api + "member/class/";
  ...
  onUpdate() {
    this.loading = true;
    const url = environment.api + "member/class/update";
  ...
  onDelete() {
    if (confirm("Delete this checklist?")) { 
      this.loading = true;
      const url = environment.api + "member/class/delete";
      const body = this.items;
      this.http.post<any>(url, body, {
        headers: this.configService.headers(),
      }).subscribe(
  ...
  onSubmit() {
    this.loading = true;
    const url = environment.api + "member/class/create";
    const body = {
      model: this.model,
    };
    this.http.post<any>(url, body, {
