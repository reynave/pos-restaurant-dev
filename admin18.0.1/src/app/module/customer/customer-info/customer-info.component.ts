...
export class CustomerInfoComponent  implements OnInit {
  ...
  htppSelect() {
    this.loading = true;
    const url = environment.api + "customer/info/select";
    this.http.get<any>(url, {
      ...
      data => {
        console.log(data);
        this.loading = false;
        this.infoGrpSelect = data['info_grp'];
        this.httpGet(); 
      },
      error => {
        console.log(error);
  ...
  httpGet() {
    this.loading = true;
    const url = environment.api + "customer/info/";
    this.http.get<any>(url, {
      headers: this.configService.headers(),
    }).subscribe(
      data => {
  ...
  onUpdate() {
    this.loading = true;
    const url = environment.api + "customer/info/update";
  ...
  onDelete() {
    if (confirm("Delete this checklist?")) { 
      this.loading = true;
      const url = environment.api + "customer/info/delete";
      const body = this.items;
      this.http.post<any>(url, body, {
        headers: this.configService.headers(),
      }).subscribe(
  ...
  onSubmit() {
    this.loading = true;
    const url = environment.api + "customer/info/create";
    const body = {
      model: this.model,
    };
    this.http.post<any>(url, body, {
