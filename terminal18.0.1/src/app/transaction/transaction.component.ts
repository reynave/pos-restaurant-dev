...
export class TransactionComponent implements OnInit {
  ...
  fnVoid(id: string) {
    this.router.navigate(['void'], {
      queryParams: { id: id, module: 'transaction', action: 'voidPaid' },
    });
  ...
  loadItems() {
    this.loading = true;
    const startDateStr = `${this.startDate.year}-${String(
      this.startDate.month
    ).padStart(2, '0')}-${String(this.startDate.day).padStart(2, '0')}`;
    const endDateStr = `${this.endDate.year}-${String(
      this.endDate.month
    ).padStart(2, '0')}-${String(this.endDate.day).padStart(2, '0')}`;
